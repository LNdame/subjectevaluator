...
class SectionTwo extends StatefulWidget{
  ...
  SectionTwo({Key key,this.course, this.lecturer, this.survey}):assert(course!=null),
        assert(lecturer!=null),
        super(key:key);


  @override
  _SectionTwoState createState() => new _SectionTwoState();
...
class _SectionTwoState extends State<SectionTwo> {
  ...
  void getSurveyData(){
    ...
    Firestore.instance.collection('surveys').document(widget.survey.Id).get().then(
          ...
          if(docSnap.data['section_two_loaded']==false){
            ...
            Firestore.instance.runTransaction((Transaction transaction) async{
              await transaction.update(docSnap.reference, {"section_two_loaded":true});
            });
  ...
  void updateSection(){
    Firestore.instance.collection('surveys').document(widget.survey.Id).get().then(
            (DocumentSnapshot docSnap){
          Firestore.instance.runTransaction((Transaction transaction) async{
            await transaction.update(docSnap.reference, {"section_two_completed":true});
          });
        });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("S2 - Lecturer and Student Relationships"),
        actions: <Widget>[
          // icon
          ...
          Expanded(
            child:   StreamBuilder(
                stream: Firestore.instance.collection('answers').where('survey_id', isEqualTo:widget.survey.Id )
                    .where('section_number', isEqualTo:2 ).snapshots(),
              ...
              children: <Widget>[
                ...
                FlatButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=> SectionOne(survey: widget.survey, course: widget.course, lecturer: widget.lecturer,)));
