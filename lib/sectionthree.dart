...
class SectionThree extends StatefulWidget{
  ...
  SectionThree({Key key,this.course, this.lecturer, this.survey}):assert(course!=null),
        assert(lecturer!=null),
        super(key:key);

  @override
  _SectionThreeState createState() => new _SectionThreeState();
...
class _SectionThreeState extends State<SectionThree>  {
  ...
  void getSurveyData(){
    ...
    Firestore.instance.collection('surveys').document(widget.survey.Id).get().then(
          ...
          if(docSnap.data['section_three_loaded']==false){
            List<Question> questions =QuestionRepository.loadQuestions(QuestionSection.three);
            ...
            Firestore.instance.runTransaction((Transaction transaction) async{
              await transaction.update(docSnap.reference, {"section_three_loaded":true});
            });
  ...
  void updateSection(){
    Firestore.instance.collection('surveys').document(widget.survey.Id).get().then(
            (DocumentSnapshot docSnap){
          Firestore.instance.runTransaction((Transaction transaction) async{
            await transaction.update(docSnap.reference, {"section_three_completed":true});
          });
        });
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(child:Column(
        ...
        children: <Widget>[
          ...
          .where('section_number', isEqualTo:3 ).snapshots(),
              ...
              children: <Widget>[
                ...
                FlatButton(
                  ...
                  onPressed: () {
                    ...
                    updateSection();
                    Navigator.of(context).pop();
                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=> SectionFour(survey: widget.survey, course: widget.course, lecturer: widget.lecturer,)));
